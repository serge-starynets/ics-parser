...
export function createFileTab(
  ...
  function showParsedResult(): void {
    ...
    const preferredView =
      (panel.querySelector<HTMLInputElement>(`input[name="output-format-${id}"]:checked`)?.value as
        | ResultTab
        | undefined) ?? 'original';
  ...
  parseButton.addEventListener('click', async () => {
    ...
    try {
      ...
      handleParseFailure(
        error instanceof Error ? error.message : 'An unexpected parsing error occurred.',
      );
    } finally {
